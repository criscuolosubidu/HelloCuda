...
// a: N x 1, b: N x 1, y = sum(elementwise_mul(a, b))
template<const int NUM_THREADS = 256>
__global__ void dot_prod_f32_f32_kernel(const float *a, const float *b, float *y, int N) {
    int tid = threadIdx.x;
...
int main() {
    ...
    dot_prod_f32_f32_kernel<256><<<N / 256, 256>>>(d_a, d_b, d_y, N);
    cudaDeviceSynchronize();
