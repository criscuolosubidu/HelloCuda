...
__global__ void dot_prod_f16_f32_kernel(half *a, half *b, float *y, int N) {
    ...
    half prod = idx < N ? __hmul(a[idx], b[idx]) : __float2half(0.0f);
    int warp = tid / WARP_SIZE;
    int lane = tid % WARP_SIZE;
    float prod_f32 = warp_reduce_sum_f16_f32<WARP_SIZE>(prod);
    if (lane == 0) reduce_sum[warp] = prod_f32;
    __syncthreads();
...
__global__ void dot_prod_f16x2_f32_kernel(half *a, half *b, float *y, int N) {
    ...
    int warp = tid / WARP_SIZE;
    int lane = tid % WARP_SIZE;
    float prod_f32 = warp_reduce_sum_f16_f32<WARP_SIZE>(prod_f16);
    if (lane == 0) reduce_sum[warp] = prod_f32;
    __syncthreads();
    float sum = lane < NUM_WARPS ? reduce_sum[lane] : 0.0f;
    if (warp == 0) sum = warp_reduce_sum_f32<NUM_WARPS>(sum);
...
__global__ void dot_prod_f16x8_f32_kernel(half *a, half *b, float *y, int N) {
    int tid = threadIdx.x;
    ...
    const half half_zero = __float2half(0.0f);
    half prod_f16 = half_zero;
#pragma unroll
    for (int i = 0; i < 8; i += 2) {
        half2 v = __hmul2(HALF2(pack_a[i]), HALF2(pack_b[i]));
        prod_f16 = idx + i < N ? __hadd(prod_f16, __hadd(v.x, v.y)) : half_zero;
    }
    int warp = tid / WARP_SIZE;
    int lane = tid % WARP_SIZE;
    float prod_f32 = warp_reduce_sum_f16_f32<WARP_SIZE>(prod_f16);
...
int main() {
    ...
    transform_f32_f16_kernel<<<num_blocks, 256>>>(d_a, d_a_f16, N);
    transform_f32_f16_kernel<<<num_blocks, 256>>>(d_b, d_b_f16, N);
    ...
    // dot_prod_f16_f32_kernel<<<num_blocks, 256>>>(d_a_f16, d_b_f16, d_y, N); // 0.0874 ms
    // dot_prod_f16x2_f32_kernel<<<num_blocks, 256 / 2>>>(d_a_f16, d_b_f16, d_y, N); // 0.0904 ms
    dot_prod_f16x8_f32_kernel<<<num_blocks, 256 / 8>>>(d_a_f16, d_b_f16, d_y, N); // 0.0772 ms
    cudaDeviceSynchronize();
    ...
    std::cout << "CPU Compute dot-product Result (float32) : " << ans << std::endl;

    cudaMemcpy(&h_y, d_y, sizeof(float), cudaMemcpyDeviceToHost);
    std::cout << "GPU Compute dot-product Result (half -> float32): " << h_y << std::endl;
