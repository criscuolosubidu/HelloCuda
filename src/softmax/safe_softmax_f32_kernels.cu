...
__global__ void safe_softmax_f32_per_token_kernel(float *x, float *y, int N) {
    ...
    float mx_val = idx < N ? x[idx] : -FLT_MAX;
    mx_val = block_reduce_max_f32<NUM_THREADS>(mx_val);
    float exp_val = idx < N ? expf(x[idx] - mx_val) : 0.0f;
...
__global__ void safe_softmax_f32x4_per_token_kernel(float *x, float *y, int N) {
    ...
    float mx_val = reg_x.x;
    mx_val = fmaxf(mx_val, reg_x.y);
    mx_val = fmaxf(mx_val, reg_x.z);
    mx_val = fmaxf(mx_val, reg_x.w);
    mx_val = block_reduce_max_f32<NUM_THREADS>(mx_val);
    float4 reg_exp;
    reg_exp.x = idx < N ? expf(reg_x.x - mx_val) : 0.0f;
    reg_exp.y = idx < N ? expf(reg_x.y - mx_val) : 0.0f;
    reg_exp.z = idx < N ? expf(reg_x.z - mx_val) : 0.0f;
    reg_exp.w = idx < N ? expf(reg_x.w - mx_val) : 0.0f;
...
int main() {
    ...
    // safe_softmax_f32_per_token_kernel<<<TOKENS, TOKEN_DIM>>>(d_x, d_y, N); // 55.5x speed up
    safe_softmax_f32x4_per_token_kernel<<<TOKENS, TOKEN_DIM / 4>>>(d_x, d_y, N); // 95.4428x speed up
    cudaDeviceSynchronize();
