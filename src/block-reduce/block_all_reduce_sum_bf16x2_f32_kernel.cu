...
__global__ void block_all_reduce_sum_bf16x2_f32_kernel(__nv_bfloat16* a, float *v, int N) {
    int tid = threadIdx.x;
    int idx = (blockIdx.x * NUM_THREADS + threadIdx.x) * 2;
    constexpr int NUM_WARPS = (NUM_THREADS + WARP_SIZE - 1) / WARP_SIZE;
    __shared__ float reduce_sum[NUM_WARPS];
    __nv_bfloat162 reg_a = BFLOAT2(a[idx]);
...
int main() {
    ...
    // block_all_reduce_sum_bf16x2_bf16_kernel<<<256 * 256, 256 / 2>>>(d_x, d_y, N); // 4.3899 ms
    block_all_reduce_sum_bf16x2_f32_kernel<<<256 * 256, 128>>>(d_x, d_y, N); // 4.3116 ms
