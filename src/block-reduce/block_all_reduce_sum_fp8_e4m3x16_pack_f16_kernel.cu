...
__global__ void block_all_reduce_sum_fp8_e4m3x16_pack_f16_kernel(__nv_fp8_storage_t *a, float *y, int N) {
    int tid = threadIdx.x;
    int idx = (blockIdx.x * NUM_THREADS + tid) * 16; // 8 * 16 = 128 = 32 * 4
    constexpr int NUM_WARPS = (NUM_THREADS + WARP_SIZE - 1) / WARP_SIZE;
    __shared__ half reduce_sum[NUM_WARPS];
...
#pragma unroll
    ...
    for (int i = 0; i < 16; ++i) {
        sum_f16 = __hadd(sum_f16, __nv_cvt_fp8_to_halfraw(pack_a[i], __NV_E4M3));
    }
    int warp = tid / WARP_SIZE;
    int lane = tid % WARP_SIZE;
...
int main() {
    ...
    for (int i = 0; i < N; ++i) {
        h_x[i] = dist(gen);
        h_x_fp8[i] = __nv_cvt_float_to_fp8(h_x[i], __NV_SATFINITE, __NV_E4M3);
    }
    ...
    std::cout << "\nTesting FP8 E4M3 kernel..." << std::endl;
    auto start = std::chrono::high_resolution_clock::now();

    cudaMemcpy(d_x_fp8, h_x_fp8.data(), N * sizeof(__nv_fp8_storage_t), cudaMemcpyHostToDevice);
    cudaMemset(d_y, 0, sizeof(float));

    block_all_reduce_sum_fp8_e4m3x16_pack_f16_kernel<NUM_THREADS / 16><<<NUM_BLOCKS, NUM_THREADS / 16>>>(d_x_fp8, d_y, N);
