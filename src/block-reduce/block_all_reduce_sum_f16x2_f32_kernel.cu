...
// a: Nx1, y=sum(a)
template<const int NUM_THREADS = 256>
__global__ void block_all_reduce_sum_f16_f32_kernel(half *a, float *v, int N) {
    int tid = threadIdx.x; // block threadIdx
    int idx = blockIdx.x * NUM_THREADS + threadIdx.x; // global threadIdx
    constexpr int NUM_WARPS = (NUM_THREADS + WARP_SIZE - 1) / WARP_SIZE;
    __shared__ float reduce_sum[NUM_WARPS];
...
__global__ void block_all_reduce_sum_f16x2_f32_kernel(half *a, float *v, int N) {
    int tid = threadIdx.x;
    ...
    __syncthreads();
    float sum = lane < NUM_WARPS ? reduce_sum[lane] : 0.0f;
    if (warp == 0) sum = warp_reduce_sum_f32<NUM_WARPS>(sum); // first warp to calculate
    if (tid == 0) atomicAdd(v, sum); // first thread to write the value
}
...
int main() {
    ...
    // block_all_reduce_sum_f16_f32_kernel<<<256, 256>>>(d_x_half, d_y, N); // 1.3915 ms
    block_all_reduce_sum_f16x2_f32_kernel<<<256, 128>>>(d_x_half, d_y, N);  // 1.2865 ms
