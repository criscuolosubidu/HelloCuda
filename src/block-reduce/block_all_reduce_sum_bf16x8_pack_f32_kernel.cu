...
__global__ void block_all_reduce_sum_bf16x8_pack_f32_kernel(__nv_bfloat16 *a, float* v, int N) {
    int tid = threadIdx.x;
    int idx = 8 * (blockIdx.x * NUM_THREADS + tid);
...
int main() {
    ...
    block_all_reduce_sum_bf16x8_pack_f32_kernel<<<256 * 256, 32>>>(d_x, d_y, N); // 4.3258 ms
