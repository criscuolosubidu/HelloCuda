...
#define MAX_EXP_F32 88.3762626647949f  // 指数超过这个数值就会变为正无穷， 2.4 * 10^38
#define MIN_EXP_F32 -88.3762626647949f // 指数超过这个数值就会编程负无穷，4.16 * 10^-39
#define MAX_EXP_F16 __float2half(11.089866488461016f)
#define MIN_EXP_F16 __float2half(-9.704060527839234f)

// __nv_bfloat162， 两个 __nv_bfloat16 (x, y)
...
__global__ void sigmoid_f32_kernel(float *x, float *y, int N) {
    ...
    if (idx < N) {
        float v = x[idx];
        v = fminf(v, MAX_EXP_F32); // fminf是专门针对单精度浮点数的函数
...
__global__ void sigmoid_f32x4_kernel(float *x, float *y, int N) {
    ...
    float4 reg_x = FLOAT4(x[idx]); // 4 * 32 读取
    float4 reg_y;
...
__global__ void sigmoid_f16x8_pack_kernel(half *x, half *y, int N) {
    ...
    LDST128BITS(pack_x[0]) = LDST128BITS(x[idx]); // load 128 bits in a memory issue
#pragma unroll
    for (int i = 0; i < 8; i += 2) {
